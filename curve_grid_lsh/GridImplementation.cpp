...
double generateNumberGrid(double const& range_from,double const& range_to){      //https://en.cppreference.com/w/cpp/numeric/random/uniform_real_distribution
...
template <>
vector<double> Grid<pair<double,double>>::concatenated_vector_from_gridCurve(vector<pair<double,double>> const& curve,unsigned int const& max_vectorSize,double const& max_coord){
...
template class Grid<pair<double,double>>; //In order to not fail the compile as the compiler wants to see the data that the templated class will have.