...
string fFunctions<inputData>::binaryStringCalculator(vector<inputData> const& p,int d,int const& k,double const& w,vector<HashFunctions<inputData>> hashFunctions){
    string binaryString;
    for(int i=0; i<d; i++){
...
template class fFunctions<double>; //In order to not fail the compile as the compiler wants to see the data that the templated class will have.